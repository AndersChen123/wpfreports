...
namespace CodeReason.Reports
{
    ...
    {
        private readonly List<DocumentPage> _firstPages = new List<DocumentPage>();

        private readonly int _pageCount;

        private Size _pageSize = Size.Empty;
        private readonly List<ReportPaginator> _reportPaginators = new List<ReportPaginator>();
        ...
        public MultipleReportPaginator(ReportDocument report, IEnumerable<ReportData> data)
        {
            ...
            {
                if (rd == null) continue;
                var paginator = new ReportPaginator(report, rd);
                _reportPaginators.Add(paginator);
                var dp = paginator.GetPage(0);
        ...
        public override DocumentPage GetPage(int pageNumber)
        {
            ...
            foreach (var paginator in _reportPaginators)
            {
            ...
            DocumentPage dp;
            if (pageNumber == 0) dp = _firstPages[paginatorIndex];
            else dp = pagePaginator.GetPage(pageNumber - currentPage);
